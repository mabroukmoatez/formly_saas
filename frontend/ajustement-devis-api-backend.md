# Ajustement Devis API Backend

## Vue d'ensemble

Ce document décrit les ajustements nécessaires pour l'endpoint API des devis afin de supporter les nouveaux filtres implémentés dans le frontend.

## Endpoint concerné

```
GET /api/organization/commercial/quotes
```
...
**Logique de filtrage :**
- Filtrer les devis où `total_ttc` (ou `total_amount`) est >= `min_amount` (si fourni)
- Filtrer les devis où `total_ttc` (ou `total_amount`) est <= `max_amount` (si fourni)
- Si les deux sont fournis, appliquer les deux conditions (AND)

**Exemple de requête :**
```
GET /api/organization/commercial/quotes?min_amount=1000&max_amount=5000
```
...
**Logique de filtrage :**
- Filtrer les devis où `issue_date` est >= `date_from` (si fourni)
- Filtrer les devis où `issue_date` est <= `date_to` (si fourni)
- Si les deux sont fournis, appliquer les deux conditions (AND)
- Les dates doivent être comparées au format date (pas de comparaison de strings)

**Exemple de requête :**
```
GET /api/organization/commercial/quotes?date_from=2025-01-01&date_to=2025-12-31
```
...
|-----------|------|-------------------|-------------|
| `client_type` | string | `particulier`, `company` | Type de client associé au devis |

**Logique de filtrage :**
- Filtrer les devis où le client associé a le type `client_type`
- Le type du client se trouve généralement dans `client.type` ou `quote.client.type`
- Valeurs possibles :
...
**Exemple de requête :**
```
GET /api/organization/commercial/quotes?client_type=company
```

## Exemple de requête complète

```
GET /api/organization/commercial/quotes?page=1&per_page=12&search=client&status=sent&min_amount=1000&max_amount=5000&date_from=2025-01-01&date_to=2025-12-31&client_type=company
```
...
- Si plusieurs filtres sont fournis, un devis doit satisfaire **tous** les critères pour être inclus dans les résultats
...
- Résultat : Devis de type "company" avec montant >= 1000 et date >= 2025-01-01
