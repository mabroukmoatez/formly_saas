...
import { ArrowLeft, Download, Search, Plus, Send, Save, Loader2, Edit, Trash2, Receipt, Calendar } from 'lucide-react';
...
export const QuoteCreationContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (prefillData) {
      ...
      if (prefillData.items && prefillData.items.length > 0) {
        const mappedItems: QuoteItem[] = prefillData.items.map((item: any, index: number) => ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.success && response.data?.quote?.id && paymentSchedule.length > 0) {
        try {
          const scheduleData = {
            ...
            show_amounts: paymentOptions.showAmounts,
            show_percentages: paymentOptions.showPercentages,
          ...
          console.error('Error saving payment schedule:', scheduleErr);
          success('Devis créé avec succès (erreur lors de la sauvegarde des conditions de paiement)');
        }
      } else {
      ...
      showError('Erreur', err.message || 'Impossible de créer le devis');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const quoteId = String(responseData.quote?.id || responseData.id);
  ...
  return (
    <div className="px-[27px] py-8">
      ...
      <div className="flex flex-col gap-[42px] max-w-[1100px] mx-auto">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className={`flex items-start gap-4 px-3 py-1 rounded-[3px] bg-white border border-dashed ${isDark ? 'bg-gray-800 border-gray-600' : 'border-[#6a90b9]'}`}>
              <div className={`font-normal text-xs ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                Date d'émission: {new Date().toLocaleDateString('fr-FR', { month: 'long', day: 'numeric', year: 'numeric' })}
              </div>
            </div>
            <div className={`flex items-start gap-4 px-3 py-1 rounded-[3px] bg-white border border-dashed ${isDark ? 'bg-gray-800 border-gray-600' : 'border-[#6a90b9]'}`}>
              <Input
              ...
              />
              {!validUntil && (
                <span className={`font-normal text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                  Valable jusqu'au...
                </span>
              )}
            </div>
        ...
        {/* Items Table - Same structure as invoice */}
