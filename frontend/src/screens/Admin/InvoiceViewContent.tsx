...
import { ArrowLeft, Download, Search, Plus, Send, Save, Loader2, Edit, Trash2, Receipt, Check, ChevronDown } from 'lucide-react';
...
import { Article, InvoiceClient, Invoice } from '../../services/commercial.types';
...
export const InvoiceViewContent: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load invoice if id is provided
        if (id) {
          const invoiceResponse = await commercialService.getInvoiceById(id);
          if (invoiceResponse.success && invoiceResponse.data) {
            const invoiceData = invoiceResponse.data as any;
            setCurrentInvoice(invoiceData);

            // Populate form with invoice data
            setInvoiceNumber(invoiceData.invoice_number || '');
            setPaymentConditions(invoiceData.payment_conditions || '');

            // Set client info
            if (invoiceData.client) {
              setClient(invoiceData.client);
              setClientInfo({
                name: invoiceData.client.company_name || `${invoiceData.client.first_name || ''} ${invoiceData.client.last_name || ''}`.trim() || '',
  ...
  const handleSave = async () => {
    ...
    try {
      const invoiceData = {
        invoice_number: invoiceNumber || currentInvoice?.invoice_number,
        client_id: client?.id || currentInvoice?.client_id,
        client_name: clientInfo.name,
        client_email: clientInfo.email || currentInvoice?.client_email || currentInvoice?.client?.email,
        client_address: clientInfo.address || currentInvoice?.client_address || currentInvoice?.client?.address,
        client_phone: clientInfo.phone || currentInvoice?.client_phone || currentInvoice?.client?.phone,
        issue_date: currentInvoice?.issue_date ? new Date(currentInvoice.issue_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        payment_conditions: paymentConditions || currentInvoice?.payment_conditions,
      ...
      await commercialService.updateInvoice(id, invoiceData);
      ...
      showError('Erreur', err.message || 'Impossible de modifier la facture');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="px-[27px] py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            >
              {currentInvoice ? `Facture ${invoiceNumber}` : 'Détails de la Facture'}
            </h1>
            <p
              className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-[#6a90b9]'}`}
            >
              {currentInvoice ? 'Visualiser et modifier la facture' : 'Gérer les informations de la facture'}
            </p>
          </div>
        </div>
      </div>
                ...
                <div className={`absolute right-0 top-full mt-2 w-48 rounded-lg shadow-lg ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'} overflow-hidden z-50`}>
                  {statuses.map((status) => (
                    <button
                      ...
                      className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-2 ${status === currentInvoice.status
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className={`flex items-start gap-4 px-3 py-1 rounded-[3px] bg-white border border-dashed ${isDark ? 'bg-gray-800 border-gray-600' : 'border-[#6a90b9]'}`}>
              <div className={`font-normal text-xs ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                {t('dashboard.commercial.mes_factures.issue_date')}: {currentInvoice?.issue_date ? new Date(currentInvoice.issue_date).toLocaleDateString('fr-FR', { month: 'long', day: 'numeric', year: 'numeric' }) : new Date().toLocaleDateString('fr-FR', { month: 'long', day: 'numeric', year: 'numeric' })}
              </div>
            </div>
          </div>
      ...
      <PaymentConditionsModal
        ...
        existingPaymentTerms={paymentConditions}
      />
