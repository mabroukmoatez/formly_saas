...
export const Messagerie = (): JSX.Element => {
  ...
  if (conversationsLoading) {
    return (
      <div className={`flex w-full h-full overflow-hidden transition-colors ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
  ...
  return (
    <div className={`flex w-full h-full overflow-hidden transition-colors ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <aside className={`w-[380px] border-r flex flex-col h-full transition-colors ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-[#dbd8d8]'}`}>
        <div className="flex flex-col gap-[18px] p-6 flex-shrink-0">
          {/* Search */}
      ...
      {showNewGroupModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`bg-white dark:bg-gray-800 rounded-xl p-6 w-[500px] max-h-[600px] ${isDark ? 'text-white' : 'text-black'}`}>
            ...
            <p className="text-sm text-gray-500 mb-4">Fonctionnalité en cours de développement...</p>
            <div className="flex gap-2 justify-end">
              <Button
                variant="outline"
