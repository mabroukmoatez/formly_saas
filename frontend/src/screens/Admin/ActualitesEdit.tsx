...
interface ActualitesEditProps {
  newsId: string;
}

export const ActualitesEdit = ({ newsId }: ActualitesEditProps): JSX.Element => {
  const { isDark } = useTheme();
  const { organization } = useOrganization();
  const { navigateToRoute } = useSubdomainNavigation();
  const { updateNews, deleteNews, loading, error, uploadImage, getNewsById } = useNewsActions();
  ...
  // Charger les données de l'actualité existante
  useEffect(() => {
    const loadNews = async () => {
      try {
        ...
        showError('Erreur', 'Impossible de charger les données de l\'actualité');
      }
    };
    loadNews();
  }, [newsId]);
  ...
  const handleSubmit = async (statusToSave: 'draft' | 'published' | 'archived') => {
    ...
    try {
      ...
      if (result) {
        success('Succès', `Actualité ${statusToSave === 'published' ? 'publiée' : statusToSave === 'archived' ? 'archivée' : 'sauvegardée en brouillon'} avec succès`);
        setTimeout(() => {
          navigateToRoute('/actualites');
        }, 1000);
      } else {
        showError('Erreur', 'Impossible de modifier l\'actualité');
      }
    } catch (error) {
    ...
    <div className="relative bg-white size-full">
      ...
      <div className="absolute flex gap-[10px] items-center right-[40px] top-[90px] z-10">
        ...
        <button
          ...
          className="flex gap-[6px] items-center justify-center px-[20px] py-[8px] rounded-[32px] bg-red-500 hover:bg-red-600 disabled:opacity-50 transition-all"
        >
          <Trash2 className="w-3.5 h-3.5 text-white" />
          <p className="font-['Poppins',Helvetica] font-medium leading-[normal] not-italic text-[14px] text-nowrap text-white whitespace-pre">
            Supprimer
          </p>
        </button>
