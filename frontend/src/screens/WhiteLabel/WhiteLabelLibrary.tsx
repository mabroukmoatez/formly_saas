...
export const WhiteLabelLibrary: React.FC = () => {
  ...
  const loadEmailTemplates = async () => {
    try {
      const response = await apiService.getEmailTemplates() as any;
      if (response.success && response.data) {
  ...
  const handleEditDocument = (document: any) => {
    ...
    if (document.type === 'email' || document.subject) {
      const url = subdomain
        ? `/${subdomain}/email-template-creation?templateId=${document.id}`
        : `/email-template-creation?templateId=${document.id}`;
      window.open(url, '_blank');
    } else if (document.is_questionnaire || document.questionnaire_type) {
      const url = subdomain
  ...
  const confirmDeleteDocument = async () => {
    ...
    try {
      ...
      // Vérifier si c'est un template d'email
      const isEmail = (documentToDelete as any).type === 'email' || (documentToDelete as any).subject;
      
      if (isEmail) {
        // Supprimer le template d'email
        response = await apiService.delete(`/api/organization/white-label/library/templates/${documentToDelete.id}`);
      }
      // Si le document est lié à une session, utiliser sessionCreation.deleteDocumentEnhanced
      else if (documentToDelete.session_uuid) {
      ...
      if (response.success) {
        success(isEmail ? 'Template d\'email supprimé avec succès' : 'Document supprimé avec succès');
  ...
  const getTypeColor = (type: 'document' | 'questionnaire' | 'email') => {
    switch (type) {
      case 'document':
        return 'bg-blue-50';
      case 'questionnaire':
  ...
  const getTypeLineColor = (type: 'document' | 'questionnaire' | 'email') => {
    switch (type) {
      case 'document':
        return 'bg-blue-400';
      case 'questionnaire':
  ...
  const getTypeBadgeColor = (type: 'document' | 'questionnaire' | 'email') => {
    switch (type) {
      case 'document':
  ...
  return (
    <DashboardLayout>
                  ...
                  {filteredDocuments.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                      {filteredDocuments.map((item: any) => {
                        // Check if it's an email template
                        const isEmail = item.type === 'email' || item.subject;
                        const isQuestionnaire = item.is_questionnaire || item.questionnaire_type;
                        const itemType = isEmail ? 'email' : (isQuestionnaire ? 'questionnaire' : 'document');
                        
                        return (
                          <Card 
                            key={item.id}
                            className={`border-2 rounded-[18px] hover:shadow-lg transition-all cursor-pointer overflow-hidden relative ${isDark ? 'border-gray-700 bg-gray-800' : 'border-[#e2e2ea] bg-white'}`}
                          ...
                          >
                            ...
                            <CardContent className="p-0">
                              {/* Colored Rectangle with Lines - Simulating text lines */}
                              <div className={`h-40 rounded-t-[18px] ${getTypeColor(itemType)} flex items-center justify-center p-6`}>
                                <div className="w-full">
                                  {/* Lignes horizontales simulant du texte - style Figma avec espacement de paragraphe */}
                                  {/* Première ligne (titre) */}
                                  <div className={`h-2.5 rounded ${getTypeLineColor(itemType)} mb-5`} style={{ width: '52%', margin: '0 auto' }}></div>
                                  
                                  {/* Paragraphe 1 */}
                                  <div className="space-y-2 mb-4">
                                    <div className={`h-2 rounded ${getTypeLineColor(itemType)}`} style={{ width: '88%', margin: '0 auto' }}></div>
                                    <div className={`h-2 rounded ${getTypeLineColor(itemType)}`} style={{ width: '95%', margin: '0 auto' }}></div>
                                    <div className={`h-2 rounded ${getTypeLineColor(itemType)}`} style={{ width: '85%', margin: '0 auto' }}></div>
                                  </div>
                                  
                                  {/* Paragraphe 2 */}
                                  <div className="space-y-2">
                                    <div className={`h-1 rounded ${getTypeLineColor(itemType)}`} style={{ width: '90%', margin: '0 auto' }}></div>
                                    <div className={`h-1 rounded ${getTypeLineColor(itemType)}`} style={{ width: '78%', margin: '0 auto' }}></div>
                                  </div>
                                  
                                  {/* Deux lignes plus courtes à la fin */}
                                  <div className="flex gap-3 mt-5 justify-center">
                                    <div className={`h-2 rounded ${getTypeLineColor(itemType)}`} style={{ width: '40%' }}></div>
                                    <div className={`h-2 rounded ${getTypeLineColor(itemType)}`} style={{ width: '35%' }}></div>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="p-5 space-y-4">
                                {/* Title */}
                                <div>
                                  <h3 className={`font-bold text-base mb-1 ${isDark ? 'text-white' : 'text-[#19294a]'}`}>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <span 
                                    className={`text-xs px-3 py-1.5 rounded-full font-medium ${getTypeBadgeColor(itemType)} border-0 inline-flex items-center`}
                                  >
                                    {isEmail ? 'Email' : isQuestionnaire ? 'Questionnaire' : 'Document'}
                                  </span>
                                  {/* Badge Apprenant - affiché conditionnellement si le document/questionnaire a des apprenants */}
                                  {(item.has_learners || item.learners_count > 0 || item.assigned_learners?.length > 0) && (
                                    <span className="text-xs px-3 py-1.5 rounded-full font-medium bg-gray-100 text-gray-700 border-0 inline-flex items-center gap-1.5">
      ...
      <DeleteConfirmationModal
        ...
        type={
          (documentToDelete as any)?.type === 'email' || (documentToDelete as any)?.subject 
            ? 'custom' 
            : documentToDelete?.is_questionnaire || documentToDelete?.questionnaire_type 
              ? 'questionnaire' 
              : 'document'
        }
        ...
        customMessage={
          (documentToDelete as any)?.type === 'email' || (documentToDelete as any)?.subject
            ? 'Voulez-vous vraiment supprimer ce template d\'email ?'
            : undefined
        }
        isLoading={deleting}
      />
