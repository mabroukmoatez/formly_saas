...
export const WhiteLabelLibrary: React.FC = () => {
  ...
  const handleEditDocument = (document: Document) => {
    if (document.is_questionnaire || document.questionnaire_type) {
      const url = subdomain
        ? `/${subdomain}/questionnaire-creation?documentId=${document.id}`
        : `/questionnaire-creation?documentId=${document.id}`;
      window.open(url, '_blank');
    } else {
      const url = subdomain
  ...
  const handleDeleteDocument = async (document: Document) => {
    if (!confirm(`Êtes-vous sûr de vouloir supprimer "${document.name || document.title}" ?`)) {
      return;
    }
    ...
    try {
      ...
      // Sinon, utiliser courseCreation.deleteDocument ou un endpoint général
