...
export const FlowActionModal: React.FC<FlowActionModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Email ID is required for both email and document types (document is sent via email)
      if (emailId) {
        let finalEmailId: number | null = null;
        
        // Si c'est un UUID (string), chercher l'ID numérique correspondant dans les templates
        if (typeof emailId === 'string' && emailId.includes('-')) {
          // C'est un UUID, chercher le template correspondant pour obtenir son ID
          ...
          if (template) {
            // Vérifier si le template a un ID numérique
            if (template.id !== undefined && template.id !== null) {
              const templateId = typeof template.id === 'number' ? template.id : Number(template.id);
              if (!isNaN(templateId)) {
                finalEmailId = templateId;
              }
            }
          }
          
          if (!finalEmailId) {
            showError('Erreur', 'Le template d\'email sélectionné n\'a pas d\'ID numérique. Veuillez sélectionner un autre template ou contacter le support.');
            setSaving(false);
            return;
          }
        } else {
        ...
        flowActionData.email_id = finalEmailId;
      }

      // For document type, set dest to the document title
      if (actionType === 'document') {
        flowActionData.dest = title.trim();
      }
  ...
  // Filter email templates by search
      ...
      <Card className={`relative w-full max-w-2xl max-h-[95vh] overflow-hidden ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(95vh-200px)] space-y-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={() => {
                  setActionType('email');
                  setDestType('email');
                  // Pour Email, ouvrir la sélection si aucun template n'est sélectionné
                  if (!emailId) {
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setDestType('email');
                  // Pour Document, toujours fermer la sélection de template
                  setShowEmailTemplate(false);
                }}
                className="px-4 py-2.5 rounded-lg font-medium transition-all"
                style={
          ...
          {/* Avec - Files */}
            ...
            {selectedDocuments.length === 0 && !selectedFile && (
