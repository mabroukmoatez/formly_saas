          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
              <p className={`text-sm font-medium mb-4 ${isDark ? 'text-gray-300' : 'text-[#19294a]'}`}>
                {type === 'total' ? 'Résumé des dépenses' : 'Évolution mensuelle'}
              </p>

              {/* Summary info for total type */}
              {type === 'total' && (
                <div className="flex items-end justify-between mb-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-base font-medium" style={{ color: '#8c2ffe' }}>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-base font-medium" style={{ color: '#26c9b6' }}>
              ...
              <svg width="100%" height={chartHeight} viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="xMidYMid meet">
                ...
                {type === 'total' ? (
                  ...
                  // Single bars for environnement/humains
                  monthlyData.map((data, index) => {
                    const barSpacing = innerWidth / 12;
                    const x = padding.left + index * barSpacing + (barSpacing - barWidth) / 2;
            ...
            >
              <p className={`text-sm font-medium mb-4 ${isDark ? 'text-gray-300' : 'text-[#19294a]'}`}>
                {type === 'total' ? 'Détail des dépenses' : 'Détails par mois'}
              </p>
              <div className="space-y-3 max-h-[280px] overflow-y-auto">
                {type === 'total' && categoryData.length > 0 ? (
                  // Show category breakdown for total view
                  ...
                  // Show monthly breakdown for individual views
                  monthlyData.map((data, index) => {
                    const value = type === 'humains' ? data.humains : data.environnement;
                    return (
                      <div key={index} className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-gray-700 last:border-0">
