...
import { Article, Invoice } from '../../services/commercial.types';
...
export const InvoiceCreationModal: React.FC<InvoiceCreationModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && invoice) {
      ...
      setClientInfo({
        name: invoice.client?.company_name || `${invoice.client?.first_name || ''} ${invoice.client?.last_name || ''}`.trim() || invoice.client_name || '',
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      // If editing an existing invoice, save it first
      if (invoice?.id) {
        const invoiceData = {
        ...
        const updateResponse = await commercialService.updateInvoice(String(invoice.id), invoiceData);
        ...
        // Create new invoice first, then download PDF
        const invoiceData = {
          invoice_number: invoiceNumber,
          client_name: clientInfo.name,
          client_email: clientInfo.email,
        ...
        const createResponse = await commercialService.createInvoice(invoiceData as any);
        ...
        onSave(); // Refresh list for new invoices
      }
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (invoice?.id) {
        ...
        // Create new invoice first, then send email
        const invoiceData = {
          invoice_number: invoiceNumber,
          client_name: clientInfo.name,
          client_email: clientInfo.email,
            ...
            <div className="flex items-center justify-between gap-4">
              <div className={`flex w-[219px] h-[60px] items-center justify-center rounded-[5px] bg-white ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                {organization?.organization_logo_url ? (
                  <img src={fixImageUrl(organization.organization_logo_url)} alt="Logo" className="h-full object-contain" />
                ) : (
