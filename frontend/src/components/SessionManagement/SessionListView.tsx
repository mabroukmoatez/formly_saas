      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {/* Filter Dropdown */}
            {showFilterDropdown && (
              <div className={`absolute left-0 mt-2 w-72 rounded-xl shadow-lg border z-20 p-4 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
                }`}>
                {/* Formation Filter */}
                <div className="mb-4">
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => onFiltersChange({ ...filters, status: e.target.value as any })}
                  >
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {['à venir', 'en cours', 'terminée'].map(status => (
                      <Badge
                        key={status}
                        className={`cursor-pointer ${filters.status === status
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <Input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => onFiltersChange({ ...filters, startDate: e.target.value })}
                    />
                    ...
                    <Input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => onFiltersChange({ ...filters, endDate: e.target.value })}
                    />
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <select
                    className={`w-full h-9 rounded-lg border px-3 mb-2 ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-200'}`}
                    value={filters.type}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => onFiltersChange({ ...filters, type: e.target.value as any })}
                  >
                    ...
                    <td className={`px-4 py-3 text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                      {session.trainers && session.trainers.length > 0 ? (
                        <div className="flex items-center">
                          <span className="truncate max-w-[150px]" title={session.trainers[0].name}>
                            {session.trainers[0].name}
                          </span>
                          {session.trainers.length > 1 && (
                            <span className="text-blue-500 ml-1 font-medium text-xs">
                              +{session.trainers.length - 1}
                            </span>
                          )}
                        </div>
