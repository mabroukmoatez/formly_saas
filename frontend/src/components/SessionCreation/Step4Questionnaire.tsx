...
export const Step4Questionnaire: React.FC = () => {
  ...
  const loadAllOrganizationQuestionnaires = async () => {
    try {
      ...
      const questionnairesRes = await sessionCreation.getAllOrganizationDocuments({ 
  ...
  const handleUseTemplate = async (templateQuestionnaire: any) => {
    try {
      ...
      await sessionCreation.createDocumentEnhanced(formData.sessionUuid!, formDataToSend);
  ...
  const loadQuestionnaires = async () => {
    try {
      ...
      const response = await sessionCreation.getQuestionnaires(formData.sessionUuid!, {
        audience: 'students'
      });
      ...
      if (response.success) {
        ...
        questionnairesData = response.data?.data || response.data || [];
      }
      
      if (Array.isArray(questionnairesData) && questionnairesData.length > 0) {
  ...
  const loadTemplates = async () => {
    try {
      const response = await sessionCreation.getDocumentTemplatesEnhanced({ 
        type: 'questionnaire', 
        is_active: true 
      });
  ...
  const handleCreateQuestionnaire = async (questionnaireData: any) => {
    try {
      ...
      if (response.success) {
        ...
        const errorMsg = response.message || response.error || 'Erreur lors de la création du questionnaire';
  ...
  const handleDeleteQuestionnaire = async (questionnaireUuid: string) => {
    ...
    try {
      ...
      if (qToDelete) {
        await sessionCreation.deleteDocumentEnhanced(formData.sessionUuid!, qToDelete.id);
              ...
              <CardContent className="p-5">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className="flex-1"
                    onClick={() => {
                      const previewUrl = buildRoute(`/session/${formData.sessionUuid}/document/${quest.id}`);
