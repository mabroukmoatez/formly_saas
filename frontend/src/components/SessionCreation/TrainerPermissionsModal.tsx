...
export const TrainerPermissionsModal: React.FC<TrainerPermissionsModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    // Use UUID for trainer_id (backend expects UUID string format)
    // The backend validation requires trainer_id to be a string, and UUID format is preferred
    let trainerIdToUse: number | string = selectedTrainer.uuid || selectedTrainer.id;
    
    if (!trainerIdToUse) {
      throw new Error('ID du formateur manquant');
    }
    
    // If we have a UUID, use it (preferred). Otherwise use numeric ID converted to string
    if (selectedTrainer.uuid) {
      trainerIdToUse = selectedTrainer.uuid;
      console.log('🟢 Using UUID for trainer:', trainerIdToUse);
    } else if (selectedTrainer.id) {
      trainerIdToUse = String(selectedTrainer.id);
      console.log('🟢 Using numeric ID (as string) for trainer:', trainerIdToUse);
    }
