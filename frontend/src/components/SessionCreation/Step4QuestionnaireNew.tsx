...
import { QuestionnaireSelectionModal } from '../CourseCreation/QuestionnaireSelectionModal';
...
export const Step4QuestionnaireNew: React.FC = () => {
  ...
  useEffect(() => {
    // Use courseUuid for COURSE content operations
  ...
  const handleUseTemplate = async (templateQuestionnaire: any) => {
    try {
      // Copy the questionnaire to current session
  ...
  const loadQuestionnaires = async () => {
    try {
      setLoading(true);
      // Use COURSE API for questionnaires (separated from documents)
              ...
              <CardContent className="p-5">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className="flex-1"
                    onClick={() => {
                      const previewUrl = buildRoute(`/course/${formData.courseUuid}/questionnaire/${quest.id}`);
