    ...
    public function boot()
    {
        ...
        try {
            ...
            if ($connection) {
                ...
                View::composer('organization.common.header', function ($view) {
                    $data['organizationNotifications'] = Notification::where('user_id', @auth()->user()->id)->where('user_type', 4)->where('is_seen', 'no')->orderBy('created_at', 'DESC')->get();
                    $count = $data['organizationNotifications']->count();
                    $data['totalOrganizationNotifications'] = $count;
                    $view->with($data);
                });
            }
