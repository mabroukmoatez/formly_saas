...
{
    if ($file != '') {
        // If the file path already includes 'storage/', use it as is
        if (strpos($file, 'storage/') === 0) {
            return asset($file);
        }
        // Otherwise, prepend 'storage/' to make it accessible via the symlink
...
function getVideoFile($file)
{
    ...
    // Fallback: If the file path already includes 'storage/', use it as is
    if (strpos($file, 'storage/') === 0) {
        return asset($file);
    }
    // Otherwise, prepend 'storage/' to make it accessible via the symlink
...
if (!function_exists('hasLimitSaaS')) {
    function hasLimitSaaS($type, $package_type, $count)
    {
        if (get_option('saas_mode')) {
            ...
            // Handle unlimited (-1) or regular limits
            if ($limit == -1 || $limit >= $count + 1) {
                return true;
            }
    ...
    function hasOrganizationLimitSaaS($type, $package_type, $count, $organizationId)
    {
        if (get_option('saas_mode')) {
            // Get organization's package
            $organizationPackage = UserPackage::join('packages', 'packages.id', '=', 'user_packages.package_id')
                ->where('package_type', $package_type)
                ->where('user_packages.user_id', $organizationId)
            ...
            if ($type == PACKAGE_RULE_STUDENT) {
                $limit = @$organizationPackage->student;
            } else if ($type == PACKAGE_RULE_COURSE) {
                $limit = @$organizationPackage->course;
            } else if ($type == PACKAGE_RULE_BUNDLE_COURSE) {
                $limit = @$organizationPackage->bundle_course;
            } else if ($type == PACKAGE_RULE_SUBSCRIPTION_COURSE) {
                $limit = @$organizationPackage->subscription_course;
            } else if ($type == PACKAGE_RULE_CONSULTANCY) {
                $limit = @$organizationPackage->consultancy;
            }
