...
class OrganizationController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'custom_domain' => ['required', 'string', 'max:50', 'regex:/^[a-z0-9-]+$/', 'unique:organizations,custom_domain'],
        ...
        try {
            ...
            $user->role = USER_ROLE_ORGANIZATION; // Organization owner should be Organization role
            $user->image =  $request->image ? $this->saveImage('user', $request->image, null, null) :   null;
            $user->save();

            // Automatically assign Organization role to the organization user
            ...
            $organization_data = [
                ...
                'organization_name' => $request->organization_name ?: $user->name . ' Organization',
                'organization_tagline' => 'Professional Learning Platform',
                'organization_description' => 'Empowering learners with quality education',
    ...
    public function changeOrganizationStatus(Request $request)
    {
        ...
        try {
            ...
            if ($request->status == STATUS_APPROVED)
            {
                ...
                // Automatically assign Organization role to the organization user when approved
                $organizationRole = Role::where('name', 'Organization')->first();
                if ($organizationRole && !$user->hasRole('Organization')) {
                    $user->assignRole('Organization');
                }
