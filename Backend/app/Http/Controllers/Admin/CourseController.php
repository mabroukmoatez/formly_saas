...
class CourseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Instructor filter
        if ($request->filled('instructor') && $request->get('instructor') !== 'all') {
        ...
        // Paginate results
        $data['courses'] = $query->paginate(25)->withQueryString();
        
        // Get filter options for dropdowns - Get ALL categories and instructors, not just those with courses
        $data['categories'] = \App\Models\Category::orderBy('name')->get();
        $data['instructors'] = \App\Models\Instructor::with('user')->orderBy('first_name')->get();
