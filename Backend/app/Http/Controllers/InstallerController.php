...
class InstallerController extends Controller
{
    ...
    public function markAllAsReadNotification(Request $request)
    {
        ...
        elseif(auth()->user()->role == USER_ROLE_INSTRUCTOR && $request->type == 2){
            Notification::where('user_id', auth()->user()->id)->where('user_type', 2)->where('is_seen', 'no')->update(['is_seen' => 'yes']);
        }
        elseif(auth()->user()->role == USER_ROLE_ORGANIZATION && $request->type == 4){
            Notification::where('user_id', auth()->user()->id)->where('user_type', 4)->where('is_seen', 'no')->update(['is_seen' => 'yes']);
        }
        else{
